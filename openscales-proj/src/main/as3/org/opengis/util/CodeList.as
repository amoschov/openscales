...
package org.opengis.util {
	...
	public class CodeList {
		...
		private static var _I18N:LocalizedClassProperties;
		{
			...
			_I18N.setLocale(Lang.EN, {"duplicated.name": "Duplicated name for codelist value : ${name}", "list.already.exists": "CodeList ${id} already exists", "list.does.not.exist": "CodeList ${id} does not exist"});
			_I18N.setLocale(Lang.FR, {"duplicated.name": "Nom dupliqué de la valeur de la liste de codes : ${name}", "list.already.exists": "La liste de codes ${id} existe déjà", "list.does.not.exist": "La liste de codes ${id} n'existe pas"});
		}
		...
		public function CodeList(values:Array, name:String, identifier:String="") {
			...
			for each (var item:CodeList in values) {
				if (item._code == name) {
					throw new DefinitionError(_I18N.translate("duplicated.name", {"name": name}));
				}
			}
			// retreive class name :
			...
			if (cl == null) {
				...
				if (neq) {
					throw new DefinitionError(_I18N.translate("list.already.exists", {"id": cn}));
				}
			}
		...
		public static function valueOf(name:String, cn:String):CodeList {
			...
			var cl:Array=CodeList.CODELISTS[cn] as Array;
			if (!cl) {
				throw new DefinitionError(_I18N.translate("list.does.not.exist", {"id": cn}));
			}
		...
		public function toString():String {
			return "[" + "name=" + this._code + (this._identifier ? " identifier=" + this._identifier : "") + "]";
		}
