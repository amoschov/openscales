...
package org.openscales.core.i18n {
	...
	public class LocalizedProperties implements IEventDispatcher {
		 ...
		 * @private
		 */
		private static var _LOCALIZED_RESOURCES:Object={DEFAULT_ID: {"en": {"already.known.id": "Localization '${id}' already exists."}, "fr": {"already.known.id": "La traduction '${id}' existe déjà."}}};
		...
		private function _setId(id:String):void {
			...
			if (_LOCALIZED_RESOURCES[this._id] != null) {
				var msg:String=LocalizedProperties.translate(DEFAULT_ID, this.preferedLang(), "already.known.id", {id: this._id});
		...
		private function _completeH(e:Event):void {
			this.setLocale(this._currentlyLoadingLang, this.loader.parse(this.loader.data));
			this.unlock();
		...
		public function set language(lang:Lang):void {
			if (lang != null && lang != this.language && !this.isLocked()) {
				if (this.contains(lang)) {
					...
					this.notifyChangeLang(this._currentlyLoadingLang == lang ? LanguageEvent.LOADED_LANG : LanguageEvent.CHANGE_LANG);
				} else {
					this.lock();
					this._currentlyLoadingLang=lang;
					if (!this.loader.hasEventListener(IOErrorEvent.IO_ERROR)) {
						this.loader.addEventListener(IOErrorEvent.IO_ERROR, this._errorH);
					}
					if (!this.loader.hasEventListener(SecurityErrorEvent.SECURITY_ERROR)) {
						this.loader.addEventListener(SecurityErrorEvent.SECURITY_ERROR, this._errorH);
					}
					if (!this.loader.hasEventListener(Event.COMPLETE)) {
						this.loader.addEventListener(Event.COMPLETE, this._completeH);
					}
		...
		public function notifyChangeLang(type:String=null):void {
			if (!this.isLocked()) {
				type=type == null ? LanguageEvent.CHANGE_LANG : type;
		...
		public function toString():String {
			var s:String="[" + getQualifiedClassName(this) + this.id ? " id=" + this.id : "";
			var ps:Object;
