package org.openscales.core
{
    ...
    public final class StringUtils {
         ...
         * @private
         */
        private static const _SPRINTF:RegExp=
            /%%|%(\d+\$)?([-+#0 ]*)(\*\d+\$|\*|\d+)?(\.(\*\d+\$|\*|\d+))?([scboxXuidfegEG])/g;
         ...
         */
         private static function _pad ( str:String, len:Number, chr:String, leftJustify:Boolean ) : String {
            var padding:String= str.length >= len?
                ""
        ...
        private static function _formatBaseX ( value:Number, base:String, prefix:Boolean, leftJustify:Boolean, minWidth:Number, precision:Number, zeroPad:Boolean ) : String {
            ...
            var svalue:String=
                sprefix +
                StringUtils._pad(nmbr.toString(parseInt(base)), precision || 0, "0", false);
        ...
        public static function sprintf ( format:String, ... args ) : String {
            ...
            return format.replace(
                ...
                function ( ) : String {
                    ...
                    var leftJustify:Boolean= false,
                        positivePrefix:String= "",
                        prefix:String= "",
                        zeroPad:Boolean= false,
                        prefixBaseX:Boolean= false,
                        nmbr:Number,
                        lflags:Number= flags!=null? flags.length : 0;
                    if (lflags>0) {
                    ...
                    var value:String= valueIndex?
                        args[parseInt(valueIndex.slice(0, -1))-1]
                    :   args[i++];
                    switch (type) {
                    case "s":
                        return StringUtils._formatString(
                            String(value), leftJustify, mW, decimals, zeroPad);
                    case "c":
                        return StringUtils._formatString(
                            String.fromCharCode(parseInt(value)),
                            leftJustify, mW, decimals, zeroPad);
                    case "b":
                        return StringUtils._formatBaseX(
                            parseInt(value), "2", prefixBaseX, leftJustify, mW, decimals, zeroPad);
                    case "o":
                        return StringUtils._formatBaseX(
                            parseInt(value), "8", prefixBaseX, leftJustify, mW, decimals, zeroPad);
                    case "x":
                        return StringUtils._formatBaseX(
                            parseInt(value), "16", prefixBaseX, leftJustify, mW, decimals, zeroPad);
                    case "X":
                        return StringUtils._formatBaseX(
                            parseInt(value), "16", prefixBaseX, leftJustify, mW, decimals, zeroPad
                        ).toUpperCase();
                    case "u":
                        return StringUtils._formatBaseX(
                            parseInt(value), "10", prefixBaseX, leftJustify, mW, decimals, zeroPad);
                    case "i":
                    case "d": {
                        nmbr= parseInt(value);
                        ...
                        value=
                            prefix +
                            StringUtils._pad(String(Math.abs(nmbr)), decimals, "0", false);
                    ...
                    case "G": {
                        ...
                        return StringUtils._justify(
                            value, prefix, leftJustify, mW, zeroPad
                        )[textTransform]();
                    }
                    default:
                        return subStr;
